...
import {FetchDataAction, FetchDataResponse} from '../../types';
...
function* fetchData(action: PayloadAction<FetchDataAction>) {
  const {type} = action.payload;
  try {
    yield put(setStatusPending({type}));
    const response: FetchDataResponse = yield call<any>(axios.get, [
      `${TMDB_BASE}/trending/${type}/week?api_key=${KEY}&page=1`
    ]);
